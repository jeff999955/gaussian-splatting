...
def loadPerspectiveIntrinsic(filename):
    ...
    for intrinsic in intrinsics:
        if intrinsic.startswith("P_rect"):
            Tr[intrinsic] = np.concatenate(
                (readVariable(fid, intrinsic, 3, 4), lastrow)
            )
        else:
...
if __name__ == "__main__":
    if "KITTI360_DATASET" in os.environ:
        ...
        kitti360Path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "..", ".."
        )

    fileCameraToPose = os.path.join(
        kitti360Path, "calibration", "calib_cam_to_pose.txt"
    )
    Tr = loadCalibrationCameraToPose(fileCameraToPose)
    print("Loaded %s" % fileCameraToPose)
    print(Tr)

    fileCameraToVelo = os.path.join(
        kitti360Path, "calibration", "calib_cam_to_velo.txt"
    )
